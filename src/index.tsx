...
EventEmitter.addListener("@_RNS3_Events", async (event: { task: any; error: any; }) => {
  ...
  if (task) {
    ...
    if (Platform.OS === "ios") {
      ...
      finalTask = await setTaskExtra(task, { state, bytes, totalBytes }, false);
    } else if (Platform.OS === "android") {
      const { bytes } = task;
      finalTask = await setTaskExtra(task, { bytes }, false);
    }
    if (listeners[task.id]) {
      listeners[task.id].forEach((cb: (arg0: any, arg1: any) => any) => cb(error, finalTask));
    }
  }
});
...
async function getTaskExtras() {
  try {
    taskExtras = await AsyncStorage.getItem(storeKey) || {};
  } catch (e) {
    taskExtras = {};
  }
  return taskExtras;
...
function saveTaskExtras() {
  return AsyncStorage.setItem(storeKey, taskExtras);
}

async function setTaskExtra(task: { id: any; }, values: { state?: any; bytes?: any; totalBytes?: any; bucket?: any; key?: any; others?: {} | {}; }, isNew: boolean | undefined) {
  const { id } = task;
...
export async function setupWithBasic(options = {}) {
  ...
  const result = await S3Multipart.setupWithBasic({ ...defaultOptions, ...opts});
  if (result) {
    await getTaskExtras();
    S3Multipart.initializeRNS3();
  }
...
export async function setupWithCognito(options = {}) {
  ...
  const result = await S3Multipart.setupWithCognito({ ...defaultCognitoOptions, ...opts });
  if (result) {
    await getTaskExtras();
    S3Multipart.initializeRNS3();
  }
